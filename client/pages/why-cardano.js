...
export default function WhyCardano() {
  return (
    <Layout date={page.date} title={page.title}>
      ...
      <p><strong>Problem case (vulnerability of custodial stake):</strong> In June 2021, crypto custody provider <ExternalLink href="https://cryptoslate.com/second-largest-eth2-staking-pool-sues-fireblocks-after-75-million-ethereum-blunder/">Fireblocks lost $75 million worth of Eth</ExternalLink> held for the StakeHound stake pool.</p>
      ...
      <p><strong>Problem case 1 (paying fees for failed transactions):</strong> Earlier in October 2021, an <ExternalLink href="https://www.theblockcrypto.com/post/120438/ethereum-user-pays-430000-in-transaction-fees-for-a-failed-payment">Ethereum user spent $430,000 in fees for a transaction that ultimately failed</ExternalLink>.</p>
      ...
      <p><strong>Problem case 2 (exploiting global state):</strong> The <ExternalLink href="https://www.gemini.com/cryptopedia/the-dao-hack-makerdao">DAO hack of 2016 resulted in the hard fork of Ethereum</ExternalLink>, reversing transactions, and splitting off a separate blockchain, Ethereum Classic.</p>
      ...
      <p><strong>Problem case (transactions dependent on a specific native coin):</strong> A colleague once tried to swap USDT for another ERC20 token on Uniswap. However, their wallet didn‘t have any ETH to cover the transaction fee, and they weren‘t even able to change a small amount of USDT into ETH to enable the swap, because that required ETH too! Instead, they purchased ETH on a centralized exchange and moved it to their wallet in order to move forward with the transaction on Uniswap.</p>
      <p>Babel fees avoids this scenario completely. Given a native token used in a swap, transaction fees can be paid in that token as well.</p>
      ...
      <p><strong>Problem case (full network outage during unbound demand):</strong> In September 2021, <ExternalLink href="https://beincrypto.com/solana-down-developers-restart-mainnet-beta/">Solana experienced a network outage lasting 17 hours</ExternalLink>, until the network could be upgraded and restarted.</p>
      ...
      <p>The isomorphic property of Hydra state channels allows the ledger model and transactions to behave off-chain exactly the same way as they do on-chain. This property, along with the characteristic that Hydra extends processing capability on top of the main chain, has the benefit that implementing Hydra will not require a significant upgrade of the main chain (such as in sharding, and possibly rollups). Additionally, Hydra participants can be comprised of existing trusted stake pool operators. As the network scales to process more transactions, the number of nodes and hydra participants can scale as well to meet the demand.</p>
